...
namespace TemplateDomain.ReadModel.Queries.RavenDB.IntegrationTests
{
    public class OrganizationSearchQueryTests : IClassFixture<DocumentStoreFixture>
    {
        ...
        public async Task Should_Execute()
        {
            ...
            var res = await qry.Execute(new PaginatedQueryRequest { Qry = new Dictionary<string, string> { { OrganizationQueriesKeys.SearchKey,"*"} }, CurrentPage = 0, PageSize = 10 });
        ...
        {
            var qry = new OrganizationQueries(DocumentStore);
            var res = await qry.Execute(new PaginatedQueryRequest { Qry = new Dictionary<string, string> { { OrganizationQueriesKeys.SearchKey, "*" } }, CurrentPage = 100, PageSize = 10 });
            Assert.Equal(2, res.Data.Count);
        }
