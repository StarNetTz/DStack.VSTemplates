    ...
    public class QueryServiceBase : Service
    {
        ...
        protected async Task<PaginatedResult<T>> GetById<T>(PaginatedQueryRequest req)
        {
            ...
            return c == null ?
                ...
                PaginatedResult<T>.CreateFrom(c);
        }
        ...
        protected async Task<PaginatedResult<T>> GetByIds<T>(PaginatedQueryRequest req)
        {
            ...
            return c == null ? new PaginatedResult<T>() : new PaginatedResult<T>() { PageSize = c.Count, TotalItems = c.Count, CurrentPage = 0, TotalPages = 1, Data = c.Values.Where(x => x != null).ToList() };
        }
